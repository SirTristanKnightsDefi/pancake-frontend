...
import { useSquire, useKnight, useLegend, useTable, useWbnb } from 'hooks/useContract'
...
import ApproveWbnbButton from 'components/ApproveWbnbButton'
...
type State = {
  ...
  rawWbnbBalance: number
  wbnbBalance: string
  wbnbAllowance: number
}
...
const MilfNftCard: React.FC<NftCardProps> = ({ nft }) => {
  ...
  useEffect(() => {
    const fetchTokensData = async (acct: string) => {
      const kdfnNFTsAddress = getMilfNFTsAddress()
      let squireBalance = '0'
      ...
      if(account){
        ...
        rawWbnbBalance = await wbnbContract.methods.balanceOf(account).call()
        wbnbAllowance = await wbnbContract.methods.allowance(account, kdfnNFTsAddress).call()
        const calculatedWbnbBalance = rawWbnbBalance / 1e18
        wbnbBalance = calculatedWbnbBalance.toFixed(4)
      }
  ...
  }, [fastRefresh, account, squireContract, knightContract, legendContract, tableContract, wbnbContract])
  ...
  const [onPresentPurchaseWithAmountModal] = useModal(
    <PurchaseNftWithAmountModal nft={nft} amount={state.purchaseTokenAmount} tokenIds={tokenIds} onSuccess={handleSuccess} />,
  )
  ...
  return (
    <Card isActive={walletOwnsNft}>
      ...
      <CardBody>
        ...
        {account && state.mintable && nft.purchaseTokenName === "WBNB" ? (
            <Text>You Hold: {state.wbnbBalance} WBNB</Text>     
          ) : (
            <Text/>
        )}
        ...
        {account && state.mintable && nft.purchaseTokenName === "WBNB" && state.wbnbAllowance > 0 ? (
            <Button variant="secondary" mt="24px" onClick={onPresentPurchaseWithAmountModal}>
              {TranslateString(999, 'Purchase')}
            </Button>            
          ) : (
            <Text />
